...
![Basic waveform shapes](img/shapes.png)

The oscillators have independent settings (as seen in the definition
of the `settings` structure).
- `shape`: Shape of the generated waveform. The basic values are:
  - Pulse (0.0), with duty cycle of 10%
  - Square (1.0)
  - Sawtooth (2.0)
  - Triangle (3.0)
  - Sine (4.0)
...
Shape values between 0.0 and 1.0, between 2.0 and 3.0, and between 3.0 and 4.0
are interpreted as intermediate waveforms. The exact meaning depends on the base
shapes at the ends of the range:
- transition between Pulse and Square changes the Pulse duty cycle,
- transition between Sawtooth and Triangle results in a gradual change of the
  inclination of the slope,
- transition between Triangle and Sine is implemented through the simple
  interpolation of the waveforms generated by the two shapes.
...
possible. We don't know whether this was a premature optimisation or
not.
